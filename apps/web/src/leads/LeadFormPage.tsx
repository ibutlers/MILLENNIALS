...
const labels = {
  access_request: { title: 'Solicitar acceso a Realstate', eyebrow: 'Acceso privado futuro', intro: 'Déjanos tus datos para valorar una futura invitación cuando la zona privada esté disponible.', cta: 'Enviar solicitud de acceso' },
  ...
  general_contact: { title: 'Contactar con Realstate', eyebrow: 'Contacto general', intro: 'Usa este canal para consultas generales sobre la firma o la plataforma.', cta: 'Enviar consulta' }
};
...
export function LeadFormPage({ kind }: { kind: LeadKind }) {
  ...
  useEffect(() => { setPageMetadata(`${text.title} | Realstate`, `${text.intro} Captación provisional con privacidad pendiente de revisión legal.`); }, [text.intro, text.title]);
        ...
        <section className="mt-8 grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="border border-carbon/10 bg-white/70 p-5 shadow-xl shadow-carbon/5">
            ...
            <form className="mt-5 grid gap-4" onSubmit={onSubmit} noValidate>
              ...
              <label className="flex gap-3"><input name="privacyAccepted" type="checkbox" disabled={Boolean(disabledReason) || submitting} /> <span>Acepto la <Link to="/privacidad" className="underline">información de privacidad provisional</Link> para que Realstate pueda responder.</span></label>
