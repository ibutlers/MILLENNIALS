...
describe('public opportunities catalog', () => {
  ...
  it('renders cards, badges, financial metrics, progress and disclaimer from the API', async () => {
    ...
    expect(cardText.indexOf('Inversión')).toBeGreaterThanOrEqual(0);
    ...
    expect(within(card).queryByText(/inversión cubierta/i)).not.toBeInTheDocument();
    ...
    const zeroProgressBar = within(zeroProgressCard).getByRole('progressbar', { name: /inversión/i });
...
describe('public opportunity detail', () => {
  ...
  it('renders a visual opportunity sheet without inventing absent private content', async () => {
    ...
    expect(screen.getByRole('heading', { name: /^información$/i })).toBeInTheDocument();
    expect(screen.getByText(/estado del proyecto/i)).toBeInTheDocument();
    expect(screen.queryByRole('heading', { name: /resumen|descripción pública|claves del proyecto|próximos pasos/i })).not.toBeInTheDocument();
    expect(screen.getByText(/680\.000\s*€/)).toBeInTheDocument();
    const summary = screen.getByRole('complementary', { name: /datos clave/i });
    expect(within(summary).getByText(/^inversión$/i)).toBeInTheDocument();
    expect(within(summary).queryByText(/^estado$/i)).not.toBeInTheDocument();
    expect(within(summary).queryByText(/^estructura$/i)).not.toBeInTheDocument();
    expect(within(summary).queryByText(/^tipo de activo$/i)).not.toBeInTheDocument();
    expect(within(summary).queryByText(/^estrategia$/i)).not.toBeInTheDocument();
    expect(within(summary).getByText(/^capex total$/i)).toBeInTheDocument();
    expect(within(summary).getByText(/830\.000\s*€/)).toBeInTheDocument();
    expect(within(summary).getByText(/^financiación bancaria$/i)).toBeInTheDocument();
    expect(document.body).not.toHaveTextContent(/cierre\s*no publicada/i);
    ...
    expect(screen.queryByRole('heading', { name: /riesgos|hitos/i })).not.toBeInTheDocument();
    expect(document.body).not.toHaveTextContent(/riesgo urbanístico|cierre del anteproyecto/i);
    expect(screen.queryByRole('heading', { name: /media disponible|imágenes adicionales/i })).not.toBeInTheDocument();
    ...
    expect(screen.queryByRole('link', { name: /coinvierte con nosotros/i })).not.toBeInTheDocument();
    expect(screen.getByRole('link', { name: /solicitar acceso al club/i })).toHaveAttribute('href', '/acceso#solicitud');
